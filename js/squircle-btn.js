// Squircle button factory — SVG-based 3D press buttons.
// Usage: window.createSquircleBtn({ c, l, i, h, sq, floating, class })
(() => {
  ...
  window.createSquircleBtn = (cfg) => {
    const el = document.createElement("div");
    el.className = `inline-block no-select cursor-pointer ${cfg.class || ""}`;
    let p = 0, w = 0, u = Math.random().toString(36).slice(2);

    const render = () => {
      const { c = "blue", l = "", i = "", h = 44, sq = false, floating = false } = cfg;
      ...
      if (!w) {
        ...
        w = sq ? 48 : Math.ceil((CTX.measureText(label).width + (i ? 100 : 80)) * 1.1);
      }
      ...
      el.style.width = el.classList.contains("w-full") ? "100%" : `${(w + 10) * scale}px`;
      ...
      el.innerHTML = `
        <svg viewBox="0 0 ${w + 10} 60" style="width:100%;height:100%;overflow:visible" preserveAspectRatio="none">
          ...
          <text x="${5 + w / 2}" y="${20 + faceY}" text-anchor="middle" dominant-baseline="central" fill="${white ? "#3b82f6" : "#fff"}" style="pointer-events:none;font-weight:900">
            ...
            ${!sq ? `<tspan dx="${i ? 8 : 0}" dy="0" font-size="15" style="letter-spacing:1px;font-family:system-ui">${label}</tspan>` : ""}
          </text>
        </svg>`;
    };

    const press = (v) => { p = v; render(); };
    el.onpointerdown = () => press(1);
    el.onpointerup = el.onpointerleave = () => press(0);
    new ResizeObserver(() => {
      if (el.classList.contains("w-full")) {
        w = el.offsetWidth / ((cfg.h || 44) / 40) - 10;
        render();
      }
    }).observe(el);
    render();
